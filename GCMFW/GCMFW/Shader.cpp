...
void	Shader::LoadTextShader(std::string name, ShaderType type) 
{
	...
	if (sceCgcCompileString(cgCompiler, LoadShaderFile(name).c_str(), type == VERTEX_SHADER ? "sce_vp_rsx" : "sce_fp_rsx", NULL, NULL, bin, 0, output, &include) != SCECGC_OK)
	{
...
void	FragmentShader::SetParameter(std::string name, float*data) 
{
	...
	//DON'T try to set a non-existent parameter. GCM will instantly fall over.
	if (p) 
	{
