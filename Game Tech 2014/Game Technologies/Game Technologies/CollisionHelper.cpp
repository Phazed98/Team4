...
void CollisionHelper::AddCollisionImpulse(PhysicsNode& p0, PhysicsNode& p1, CollisionData& data) {
...

	{
		...
		float normDiv = (p0.GetInverseMass() + p1.GetInverseMass()) +
			Vector3::Dot(data.m_normal,
				Vector3::Cross(p0.GetInverseInertiaMatrix()*Vector3::Cross(r0, data.m_normal), r0) +
				Vector3::Cross(p1.GetInverseInertiaMatrix()*Vector3::Cross(r1, data.m_normal), r1));
		...
		Vector3 a0 = p0.GetAngularVelocity() + p0.GetInverseInertiaMatrix()* Vector3::Cross(r0, data.m_normal * jn);
		p0.SetAngularVelocity(a0);

		Vector3 l1 = p1.GetLinearVelocity() - data.m_normal*(jn*p1.GetInverseMass());
		p1.SetLinearVelocity(l1);
		Vector3 a1 = p1.GetAngularVelocity() - p1.GetInverseInertiaMatrix()* Vector3::Cross(r1, data.m_normal * jn);
...

	{
		...
		float tangDiv = (p0.GetInverseMass() + p1.GetInverseMass()) +
			Vector3::Dot(tangent,
			Vector3::Cross(p0.GetInverseInertiaMatrix()* Vector3::Cross(r0, tangent), r0) +
			Vector3::Cross(p1.GetInverseInertiaMatrix()* Vector3::Cross(r1, tangent), r1));

		float jt = -1* Vector3::Dot(dv, tangent) / tangDiv;

		Vector3 l0 = p0.GetLinearVelocity() + tangent*(jt*p0.GetInverseMass());
		p0.SetLinearVelocity(l0);
		Vector3 a0 = p0.GetAngularVelocity() + p0.GetInverseInertiaMatrix()* Vector3::Cross(r0, tangent * jt);
		p0.SetAngularVelocity(a0);

		Vector3 l1 = p1.GetLinearVelocity() - tangent*(jt*p1.GetInverseMass());
		p1.SetLinearVelocity(l1);
		Vector3 a1 = p1.GetAngularVelocity() - p1.GetInverseInertiaMatrix()* Vector3::Cross(r1, tangent * jt);
		p1.SetAngularVelocity(a1);
	}
